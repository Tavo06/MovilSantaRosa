...
class UpdateMemberStatusUseCase @Inject constructor(
    ...
    suspend operator fun invoke(uid: String, status: UserStatus): Result<Unit> {
        ...
        if (user.role != UserRole.ADMIN) {
            return Result.failure(
                Exception("No tienes permisos para gestionar solicitudes de registro.")
            )
        }

        return repository.updateMemberStatus(uid, status)
    }
}
